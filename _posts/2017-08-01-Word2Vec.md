...
Those calculations are the same from hidden layer to output layer. All the pairs are used to train the model to minimize the loss between output vectors and actural targets. Ultimately, we could get updated parameter matrices. Then using these matrices, we can predict the words around a given center word. Those output vectors are in fact the embedding of those words, since the vectors are unique for each word and could represent the semantic things of words.
...
##### - Batch data
First, I construct a class- "batch.py", where there is method to create generator of batched data. It will be more convenient for us to feed the data into tensorflow.
...
Aright, that's Word2Vec. Quite fun and great training for starting tensorflow!
